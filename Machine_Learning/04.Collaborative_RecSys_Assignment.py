...
from tools.recsys_utils import *

"""
 A movie RecSys based on TensorFlow 2.0
...
def cofi_cost_func(X, W, b, Y, R, lambda_):
    ...
    for i in range(nm):
        for j in range(nu):
            if R[i, j] == 1:
                J += (X[i].T @ W[j] + b[0, j] - Y[i, j]) ** 2

    J = J + np.sum(X ** 2) * lambda_ + np.sum(W ** 2) * lambda_
    J = J / 2
    return J
...
if __name__ == '__main__':
    ...
    my_ratings[
        366] = 5  # Harry Potter and the Sorcerer's Stone (a.k.a. Harry Potter and the Philosopher's Stone) (2001)
